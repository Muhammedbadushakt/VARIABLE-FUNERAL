(function () {
  ...
  const demoTombstones = [
    { id:'1', name:'oldUser', kind:'const', verdict:'DEAD', filePath:'src/app.js', fileName:'app.js', line:12, character:6, causeOfDeath:'Declared with great expectations, then quietly ignored.', epitaph:'Here lies oldUser. It meant well.', detectedAt: Date.now()-5000 },
    { id:'2', name:'debugMode', kind:'let', verdict:'DEAD', filePath:'src/config.js', fileName:'config.js', line:7, character:4, causeOfDeath:'Created during a debugging session and never removed.', epitaph:'debugMode — declared 2024, used never.', detectedAt: Date.now()-4000 },
    { id:'3', name:'unusedParam', kind:'parameter', verdict:'DEAD', filePath:'src/api.js', fileName:'api.js', line:21, character:18, causeOfDeath:'Passed in dutifully, referenced never.', epitaph:'In loving memory of unusedParam, who took up a whole line for nothing.', detectedAt: Date.now()-3000 },
    { id:'4', name:'legacyHelper', kind:'function', verdict:'DEAD', filePath:'src/utils.js', fileName:'utils.js', line:43, character:9, causeOfDeath:'Called exactly zero times since the day it was born.', epitaph:'RIP legacyHelper. The linter warned us.', detectedAt: Date.now()-2000 },
    { id:'5', name:'React', kind:'import', verdict:'DEAD', filePath:'src/main.jsx', fileName:'main.jsx', line:1, character:7, causeOfDeath:'Imported "just in case." The case never came.', epitaph:'Here lies React. It meant well.', detectedAt: Date.now()-1000 },
    { id:'6', name:'GhostService', kind:'class', verdict:'DEAD', filePath:'src/services.js', fileName:'services.js', line:31, character:6, causeOfDeath:'Designed with a full inheritance hierarchy in mind. Instantiated: never.', epitaph:'GhostService: gone, but still technically in the source tree.', detectedAt: Date.now()-800 },
    { id:'7', name:'temporaryValue', kind:'var', verdict:'DEAD', filePath:'src/legacy.js', fileName:'legacy.js', line:18, character:4, causeOfDeath:'Old-school, function-scoped, and forgotten in equal measure.', epitaph:'Here lies temporaryValue. It meant well.', detectedAt: Date.now()-500 },
    { id:'8', name:'futureFeature', kind:'const', verdict:'DEAD', filePath:'src/features.js', fileName:'features.js', line:64, character:6, causeOfDeath:'Built for a feature that was abandoned before launch.', epitaph:'futureFeature — declared 2024, used never.', detectedAt: Date.now()-300 }
  ];

  let tombstones = demoTombstones.slice();
  let buried = [];
  let activeKind = 'all', query = '', sortBy = 'file', expandedId = null;

  function updateStats() {
    statDead.textContent = String(tombstones.length);
    statBuried.textContent = String(buried.length);
    statAlive.textContent = '24';
  }
  ...
  function visibleTombstones() {
    ...
    if (query) list = list.filter(t =>
      ...
      t.causeOfDeath.toLowerCase().includes(query)
    );
  ...
  function render() {
    ...
    if (tombstones.length > 0 && list.length === 0) {
      const noMatch = document.createElement('div');
      noMatch.className = 'empty';
      noMatch.innerHTML = '<div class="fog"></div><p>No graves match your search.</p>';
  ...
  function renderDetail(t) {
    ...
    const epit=document.createElement('p'); epit.className='epitaph'; epit.textContent=t.epitaph;
    ...
    wrap.append(title,location,epit,cause,actions);

    actions.querySelector('.btn-jump').addEventListener('click',()=>{
      showCode(t);
    });
    actions.querySelector('.btn-bury').addEventListener('click',()=>{
      const ok=confirm(`Bury "${t.name}"?\n\nThis demo will remove the grave from the cemetery.`);
      if (!ok) return;
      tombstones=tombstones.filter(x=>x.id!==t.id);
      buried.push({...t, buriedAt:Date.now(), removedText:`const ${t.name} = /* dead code */;`});
      expandedId=null; render(); toast(`⚰️ ${t.name} has been buried.`);
    });
    actions.querySelector('.btn-ignore').addEventListener('click',()=>{
      tombstones=tombstones.filter(x=>x.id!==t.id);
      expandedId=null; render(); toast(`🕯️ ${t.name} was spared.`);
    });
  ...
  function renderBuriedList() {
    ...
    if (!buried.length) {
      ...
      buriedListEl.appendChild(li); return;
    }
    buried.slice().sort((a,b)=>b.buriedAt-a.buriedAt).slice(0,8).forEach(b=>{
      ...
      li.querySelector('.resurrect').addEventListener('click',()=>{
        tombstones.push({...b,verdict:'DEAD',detectedAt:Date.now()});
        buried=buried.filter(x=>x.id!==b.id);
        render(); toast(`🪦 "${b.name}" has risen again.`);
      });
  ...
  function showCode(t) {
    ...
    if (!modal) {
      ...
      modal.id='code-modal';
      modal.innerHTML='<div class="code-backdrop"></div><div class="code-card"><button class="code-close">×</button><div class="code-kicker">SOURCE LOCATION</div><h2></h2><div class="code-file"></div><pre></pre></div>';
      document.body.appendChild(modal);
      modal.querySelector('.code-backdrop').addEventListener('click',()=>modal.remove());
      modal.querySelector('.code-close').addEventListener('click',()=>modal.remove());
    }
    modal.querySelector('h2').textContent=`☠ ${t.name}`;
    modal.querySelector('.code-file').textContent=`${t.fileName}:${t.line+1}:${t.character+1}`;
    modal.querySelector('pre').textContent=`// demo source preview\nconst ${t.name} = createSomething();\n\n// No references found.\n// Variable Funeral marked this declaration as DEAD.`;
  }
